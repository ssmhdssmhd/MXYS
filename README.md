...
### v.0.0.12 (2026-09-21)
- ✅ **优化特殊播放返回行为**：特殊播放用 WebView(CatWebActivity) 打开网页播放器时，现在**按返回直接关闭**，不再触发 WebView 历史回退重载已失败的播放器页面。此前在「解析失败的播放页」按返回会 `goBack()` 重新加载同一页面，表现为「点一下返回自动又运行一次、又报解析失败」
- ✅ 版本 v0.0.12（versionCode 572），签名 v1+v2+v3 齐全
- 发布包：`MoxiTVBox.0.0.12.{时间}-leanback/mobile-arm64_v8a.apk`（[Releases](https://github.com/ssmhdssmhd/MXYS/releases) 下载）

### v.0.0.11 (2026-09-21)
- ✅ **修复特殊播放接口 URL 拼接错误导致解析失败**：默认接口 `http://114.134.184.91:5000/api/parse?url=` 本身已带 `?url=`，原逻辑再拼 `&url=<链接>` 会生成重复的空 `url` 参数，服务端取到空值返回 400「缺少必填参数: url」，导致特殊播放始终失败、回退普通播放后爱奇艺等加密源播不了。现改为：接口以 `url=` 结尾时直接拼接视频链接，不重复附加参数。已实测 `https://www.iqiyi.com/v_1dp87w90j9w.html` 经接口成功解析出 `getdata.staticfile.link/player/...`（网页播放器，WebView 打开）
- ✅ 版本 v0.0.11（versionCode 571），签名 v1+v2+v3 齐全
- 发布包：`MoxiTVBox.0.0.11.{时间}-leanback/mobile-arm64_v8a.apk`（[Releases](https://github.com/ssmhdssmhd/MXYS/releases) 下载）

### v.0.0.10 (2026-09-19)
- ✅ **新增「特殊播放」**（播放设置→特殊播放，默认关闭+可设接口地址）：开启后播放视频前先经「接口 + 视频链接」解析出实际播放地址；返回的 `url` 为**网页播放器**时用内置 WebView(CatWebActivity) 打开播放，为直连媒体时替换地址直接播，解析失败自动回退原播放。带短超时(5s)+失败熔断，避免拖住播放。实测接口 `http://114.134.184.91:5000/api/parse?url=` 可解析 `getdata.staticfile.link/player/...` 这类加密 k8 链接
- ✅ 版本 v0.0.10（versionCode 570），签名 v1+v2+v3 齐全
- 发布包：`MoxiTVBox.0.0.10.{时间}-leanback/mobile-arm64_v8a.apk`（[Releases](https://github.com/ssmhdssmhd/MXYS/releases) 下载）
